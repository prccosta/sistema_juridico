...
<table width="100%" class="Borda1N" id="TABLE2" language="javascript" background="img/fundo.jpg">
                        <tr>
                            <td class="TextoTopo">
                                Relatório de Parcelas em Atraso</td>
                        </tr>
                        <tr>
                            <td>
                                    ...
                                    <tr>
                                        <td colspan="2" vAlign="top" >
                                          <table Class="TextoNormal" width="100%">
                                                ...
                                                <select id="id_cliente" name="id_cliente" class="CampoTexto">
                                                 <option value="0">-- Selecione o cliente --</option>
                                                    <?php
														...
														$sql_id_cliente = mysql_query("SELECT *
															from cliente order by id_cliente");
														 while($res_id_cliente = mysql_fetch_array($sql_id_cliente) ) {
                                                         echo '<option value="'.$res_id_cliente['id_cliente'].'">'.$res_id_cliente['id_cliente']." | ".$res_id_cliente['nome_exib'].'</option>';
                                                        }
                                                    ?>
                                            </select></td>
                                              </tr>
                                            ...
                                            <label class="TextoVermelho10">O campo ANO é de preenchimento OBRIGATÓRIO.<br> Os outros campos podem ser preenchidos ou não.</label></td>
                                            </tr>
                                            <tr>
							...
							if( $mesano ){ $where[] = " `dtvenc` like '$mesano%'"; }
							if( $id_devedor ){ $where[] = " `devedor` = '$id_devedor'"; }
							if( $id_cliente ){ $where[] = " `escolas` = '$id_cliente'"; }
							$where[] = " `id_processo` = `id_proc_processo`";
													  
							$_pagi_sql = "SELECT *
							FROM processo as a
							LEFT JOIN parcela as b ON a.id_processo = b.id_proc_processo and dtvenc < now() and status_parcela = 2
							...
							if( sizeof( $where ) )  
									...
									$_pagi_sql .= ' ORDER BY parcela_atual';
